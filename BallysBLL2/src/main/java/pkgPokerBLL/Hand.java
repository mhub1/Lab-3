...
public class Hand {
	...
	public void AddCardToHand(int index, Card c) {
		CardsInHand.add(index, c);
	}
	...
	public Hand EvaluateHand() {
		...
		//System.out.println(ExplodedHands.size());
		for (Hand hand : ExplodedHands) {
			//hand = Hand.EvaluateHand(hand);
			//if(hand.getHandScore().getHandStrength() == eHandStrength.RoyalFlush)
			//System.out.println(hand.getHandScore().getHandStrength());
		}

		//	Figure out best hand
		Collections.sort(ExplodedHands, Hand.HandRank);
		//System.out.println("High hand is: " + ExplodedHands.get(0).getHandScore().getHandStrength());
		
		//	Return best hand.  
		//	TODO: Fix...  what to do if there is a tie?
		if(numWild(ExplodedHands.get(0)) > 0)
			for(Hand hnd : ExplodedHands)
				System.out.println("Wow " + hnd.getHandScore().getHandStrength());
	...
	public static ArrayList<Hand> ExplodeHands(Hand h) {
		...
		if(numWild(h) == 0 || numWild(h) == 5){
			ReturnHands.add(h);
			for(Hand hnd : ReturnHands)
				Hand.EvaluateHand(hnd);
			return ReturnHands;
		}
		if(numWild(h) >= 1){
			ReturnHands = (handleWilds(startWild(h), numWild(h), 0, new ArrayList<Hand>()));
		}
		/*else if(numWild(h) == 2){
			ReturnHands.addAll(handleWilds(startWild(h), numWild(h), 0, new ArrayList<Hand>()));
			ReturnHands.addAll(handleWilds(startWild(h), numWild(h), 1, ReturnHands));
		}*/
		if(numWild(h) >= 2){
			for(int i = 0 ; i < h.getCardsInHand().size(); i++){
				ReturnHands.addAll(handleWilds(startWild(ReturnHands.get(i)), numWild(h), i, new ArrayList<Hand>()));
			}
		}
		return ReturnHands;
	}
	...
	public static ArrayList<Hand> handleWilds(Hand h, int numWilds, int index, ArrayList<Hand> nList){
		ArrayList<Hand> newList = new ArrayList<Hand>();
		for(eSuit sut : eSuit.values()){
			for(eRank rnk : eRank.values()){
				if(sut != eSuit.JOKER && rnk != eRank.JOKER){
					Hand tempHand = new Hand();
					for(Card crd : h.getCardsInHand())
						tempHand.AddCardToHand(crd);
					tempHand.getCardsInHand().get(index).seteSuit(sut);
					tempHand.getCardsInHand().get(index).seteRank(rnk);
					//Collections.sort(tempHand.getCardsInHand(), Card.CardRank);
					Hand.EvaluateHand(tempHand);
					newList.add(tempHand);
				}
			}
		}
		return newList;
